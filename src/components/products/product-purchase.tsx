...
export function ProductPurchase({ product }: { product: Product }) {
  ...
  return <div className="product-detail-grid"><div className="product-gallery"><div className="gallery-main"><Image src={product.images[0]} alt={product.imageAlt} width={900} height={900} priority /></div><p>Demonstration product visual — replace with verified photography before launch.</p></div><div className="product-buy-panel"><ProductBadges product={product} /><span className="eyebrow">{product.subcategory}</span><h1>{product.name}</h1><p className="lead compact">{product.shortDescription}</p><div className="sample-rating" aria-hidden="true">★★★★★</div><ProductPrice priceUsd={product.basePriceUsd} compareAtUsd={product.compareAtPriceUsd} large /><p className="price-note">Demonstration price. Shipping, duty and taxes are not included.</p>
